...
int main() {
    ...
    int rem = total_students % comm_sz; 
    int sum = 0;

    for (int i = 0; i < comm_sz; i++) {
        sendcounts[i] = total_students / comm_sz;
        if (rem > 0) {
            sendcounts[i]++;
            rem--;
        }
    ...
    int n_local = sendcounts[my_rank];
    Student *all_students = NULL;
    Student *local_students = malloc(n_local * sizeof(Student));
    ...
    MPI_Scatterv(all_students, sendcounts, displs, student_mpi_t, 
                 local_students, n_local, student_mpi_t, 0, MPI_COMM_WORLD);
    for (int i = 0; i < n_local; i++) {
        if (local_students[i].total_marks >= 85) local_students[i].grade = 'O';
        else if (local_students[i].total_marks >= 70) local_students[i].grade = 'A';
        else if (local_students[i].total_marks >= 50) local_students[i].grade = 'B';
        else local_students[i].grade = 'F';
    }
    MPI_Gatherv(local_students, n_local, student_mpi_t, 
                all_students, sendcounts, displs, student_mpi_t, 0, MPI_COMM_WORLD);
